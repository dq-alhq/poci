...
import type { Item, TransferType } from '@/generated/prisma/client'
...
export function StokForm({ items, type }: { items: Item[]; type: TransferType }) {
                    ...
                    .map((item, i) => (
                        <div className={itemVariants({ variant: 'outline', className: 'relative' })} key={i}>
                            ...
                            <ItemActions className='absolute top-2 right-2'>
                                <Button
                                    onPress={() => setStocks(stocks.filter((stock) => stock.itemId !== item.id))}
                                    size='icon-sm'
                                    variant='destructive'
                                >
                                    <IconTrash />
